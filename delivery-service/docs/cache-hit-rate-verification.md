...
### 소규모(파일럿): tracking_number 5개 x 20회 = 100건, 1 VU 순차
...
### 대용량: tracking_number 1,000개 x 20회 = 20,000건, 20 VU 동시
...
| 성공 | **20,000건 (100%)** |
| 미스(최초 조회, DB) | 1,000건 |
| 히트(재조회, Redis) | 19,000건 |
| **히트율** | **95.00%** |
| 미스 평균 지연시간 | 60.1ms |
| 히트 평균 지연시간 | 20.0ms |
| **캐시 있음 vs 없음 속도 차이** | **히트가 미스 대비 3.0배 빠름** |
| 처리시간 | 22.25초 |
| TPS | 899.0 |

**Redis `INFO stats`로 교차 검증**(대용량 기준): `keyspace_hits:19000`, `keyspace_misses:1000`, `DBSIZE:1000` — 애플리케이션 레벨 측정과 정확히 일치. 20배 규모로 키워도 히트율과 속도 배율이 일관되게 유지됨을 확인.

**"레디스 캐시 안 쓸 때 vs 쓸 때" 속도 비교**: 캐시 미스(=DB 직접 조회, 캐시 없는 것과 동일한 경로) 평균 60.1ms, 캐시 히트(Redis) 평균 20.0ms — **캐시를 쓰면 약 3배 빠름**.
