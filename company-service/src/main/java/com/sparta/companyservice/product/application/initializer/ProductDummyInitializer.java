...
public class ProductDummyInitializer implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        if (productRepository.count() == 0) {
            ProductCategory electronics = categoryRepository.findAll(org.springframework.data.domain.Pageable.unpaged()).getContent().stream()
            ...
            if (companies.isEmpty() || electronics == null) {
                log.warn("⚠️ [CompanyService] 업체 또는 카테고리가 없어 상품 주입을 스킵합니다.");
                return;
            }

            Company producer = companies.get(0);
            ...
            saveProduct(techFactory.getCompanyId(), electronics.getCategoryId(), "기계식 키보드", 150000, "조용한 타건감의 저소음 적축", "https://example.com/keyboard.png");
            saveProduct(techFactory.getCompanyId(), electronics.getCategoryId(), "4K 모니터", 600000, "선명한 화질의 32인치 모니터", "https://example.com/monitor.png");
            saveProduct(producer.getCompanyId(), electronics.getCategoryId(), "고속 충전 케이블", 15000, "튼튼한 내구성의 C-type 케이블", "https://example.com/cable.png");
